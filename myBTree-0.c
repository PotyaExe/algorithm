	...
	if (pDelete != NULL) {
		...
		if ( (*pDelete)->left == NULL && (*pDelete)->right == NULL ) {
			...
			*pDelete = NULL;
			...
			pMaxNode = mySearchMaxNodeinLeftPart(&(*pDelete)->left);
		       	printf("\nMax node is %d\n\n", (*pMaxNode)->value);

			
			tmp = *pMaxNode;
			if ( (*pMaxNode)->left == NULL && (*pMaxNode)->right == NULL ) {
				*pMaxNode = NULL;
			} else {
				*pMaxNode = (*pMaxNode)->left;
			}
			//printf("\nTmp has MaxNode and we will delete MaxNode from BTree temporary!\n"); 

			printf("\nWe will now exchange the relationship between the node we want to delete and tmp!\n\n");
			tmp->left = (*pDelete)->left;
			tmp->right = (*pDelete)->right;
			*pDelete = tmp;
	 
			//printf("\nThis is not the leef pattern 2 you want to delete!\n"); 
		}
		free(q);
